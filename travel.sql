...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 13, 2020 at 11:14 AM
...
INSERT INTO `paket_tour` (`id_paket_tour`, `nama_paket`, `harga_paket`, `description`, `kode_travell`) VALUES
(1, 'Paket Hemat', 1000000, 'Paket Menunjungi sebuah destinasi yang indah dan segar.														', 12),
(2, 'Paket Istimewa', 5000000, 'Paket Tour ke destinasi yang menghangatkan badan', 10),
(3, 'Paket Senang', 1500000, '						Paket untuk menikmati alam yang indah dan mempesan untuk cuci mata.															', 12),
(4, 'Paket Liburan Singkat', 800000, 'Paket liburan singkat di sekitar Jogja', 10),
(5, 'Paket Honeymoon', 2000000, 'Paket liburan berdua menikmati indahnya suasana Jogja dan Sekitarnya ', 2),
(6, 'Paket Sultan', 5000000, 'Paket liburan Keliling ke banyak destinasi', 12),
(7, 'Paket Eksotis', 1500000, 'Paket liburan menikmati eksotisme wilayah pantai Gunung Kidul', 5),
(8, 'Paket Backpacker', 500000, 'Paket liburan Menikmati Yogyakarta ala backpacker', 1);
...
--
ALTER TABLE `paket_tour`
  MODIFY `id_paket_tour` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `destinasi`
  ADD CONSTRAINT `destinasi_ibfk_1` FOREIGN KEY (`id_paket_tour`) REFERENCES `paket_tour` (`id_paket_tour`);

--
-- Constraints for table `dp`
--
ALTER TABLE `dp`
  ADD CONSTRAINT `dp_ibfk_1` FOREIGN KEY (`id_paket_tour`) REFERENCES `paket_tour` (`id_paket_tour`),
  ADD CONSTRAINT `dp_ibfk_2` FOREIGN KEY (`id_destinasi`) REFERENCES `destinasi` (`id_destinasi`);
