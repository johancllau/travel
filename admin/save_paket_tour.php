...
<?php
   ...
   if (isset($_POST['submit']) and !empty($_POST['submit'])) {
    ...
    $harga    = $_POST['harga'];
      ...
      if (strlen(trim($destinasi)) == 0) {
         $falidate_date = false;
         echo "<H3 align='center'>Destinasi Harus Di Pilih<H3>";
         echo "<table border='0' align='center'>
                  <tr>
                     <td><input type='button' value='kembali' onClick='self.history.back()'></td>
                  </tr>
      ...
      if ($falidate_date) {
         $sql = "INSERT INTO paket_tour (nama_paket, harga_paket, description, id_destinasi, kode_travell) VALUES ('$nama', $harga, '$desc', $destinasi, $travel);";
   ...
   elseif (isset($_POST['edit']) and !empty($_POST['edit'])) {
     ...
     $harga     = $_POST['harga'];
      ...
      if ($falidate_date) {
         $sql = "UPDATE paket_tour SET nama_paket='$nama', harga_paket=$harga, description='$desc', id_destinasi=$destinasi, kode_travell=$travel WHERE id_paket_tour=$id_paket";
         $hasil = mysqli_query($kon, $sql);
         if (!$hasil) {
